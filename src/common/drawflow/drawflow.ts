export default class Drawflow {
  ...
  rerouteFixCurvature: any = false;

  curvature: any = 0;

  rerouteCurvatureStartEnd: any = 0;

  rerouteCurvature: any = 0;

  rerouteWidth: any = 6;

  dragPoint: any = false;

  editorSelected: any = false;

  connection: any = false;

  connectionElement: any = null;

  connectionSelected: any = null;
  ...
  forceFirstInput: any = false;

  draggableInputs: any = true;
  ...
  constructor(container: any, render = null, parent = null) {
  ...
  /* End Mobile Zoom */
  load() {
    let a = this.drawFlow[this.module];
    if (this.drawFlow[this.module]?.data) {
      for (var key in this.drawFlow[this.module]?.data) {
        this.addNodeImport(this.drawFlow[this.module].data[key], this.precanvas);
      }
  
      if(this.reroute) {
        for (var key in this.drawFlow[this.module].data) {
          this.addRerouteImport(this.drawFlow[this.module].data[key]);
        }
      }
  ...
  click(e: any): any {
    ...
    if(this.editorMode === 'fixed') {
      ...
      if(e.target.closest(".drawflow_content_node") != null) {
        this.elementSelected = e.target.closest(".drawflow_content_node").parentElement;
      }
    }
    ...
    switch (this.elementSelected.classList[0]) {
      ...
      case 'main-path':
        ...
        if(listClassConnection.length > 1){
          ...
          if(this.rerouteFixCurvature) {
            this.connectionSelected.parentElement.querySelectorAll(".main-path").forEach((item: any, i: any) => {
    ...
    if (['input','output','main-path'].includes(this.elementSelected.classList[0])) {
      e.preventDefault();
    }
  ...
  position(e: any) {
    ...
    if(this.editorSelected) {
      x =  this.canvasX + (-(this.posX - e_pos_x))
      y = this.canvasY + (-(this.posY - e_pos_y))
    ...
    if(this.drag) {
      e.preventDefault();
      var x = (this.posX - e_pos_x) * this.precanvas.clientWidth / (this.precanvas.clientWidth * this.zoom);
      var y = (this.posY - e_pos_y) * this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom);
      this.posX = e_pos_x;
      this.posY = e_pos_y;

      this.elementSelected.style.top = (this.elementSelected.offsetTop - y) + "px";
      this.elementSelected.style.left = (this.elementSelected.offsetLeft - x) + "px";

      this.drawFlow[this.module].data[this.elementSelected.id.slice(5)].pos_x = (this.elementSelected.offsetLeft - x);
      this.drawFlow[this.module].data[this.elementSelected.id.slice(5)].pos_y = (this.elementSelected.offsetTop - y);
    ...
    if(this.dragPoint) {
      ...
      var pos_x = this.posX * ( this.precanvas.clientWidth / (this.precanvas.clientWidth * this.zoom)) - (this.precanvas.getBoundingClientRect().x * ( this.precanvas.clientWidth / (this.precanvas.clientWidth * this.zoom)));
      var pos_y = this.posY * ( this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom)) - (this.precanvas.getBoundingClientRect().y * ( this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom)));

      this.elementSelected.setAttributeNS(null, 'cx', pos_x);
      this.elementSelected.setAttributeNS(null, 'cy', pos_y);

      const nodeUpdate = this.elementSelected.parentElement.classList[2].slice(9);
      const nodeUpdateIn = this.elementSelected.parentElement.classList[1].slice(13);
      const output_class = this.elementSelected.parentElement.classList[3];
      const input_class = this.elementSelected.parentElement.classList[4];

      let numberPointPosition = Array.from(this.elementSelected.parentElement.children).indexOf(this.elementSelected)-1;

      if(this.rerouteFixCurvature) {
        const numberMainPath = this.elementSelected.parentElement.querySelectorAll(".main-path").length-1;
      ...
      const searchConnection = this.drawFlow[this.module].data[nodeId].outputs[output_class].connections.findIndex(function(item: any, i: number) {
        return item.node ===  nodeUpdateIn && item.output === input_class;
      });

      this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points[numberPointPosition] = { pos_x: pos_x, pos_y: pos_y };

      const parentSelected = this.elementSelected.parentElement.classList[2].slice(9);
  ...
  dragEnd(e: any) {
    ...
    if(this.drag) {
      if(this.posXStart != e_pos_x || this.posYStart != e_pos_y) {
    ...
    if(this.dragPoint) {
      this.elementSelected.classList.remove("selected");
        if(this.posXStart != e_pos_x || this.posYStart != e_pos_y) {
          this.dispatch('rerouteMoved', this.elementSelected.parentElement.classList[2].slice(14));
        }
    }

    if(this.editorSelected) {
      this.canvasX = this.canvasX + (-(this.posX - e_pos_x));
      this.canvasY = this.canvasY + (-(this.posY - e_pos_y));
      this.editorSelected = false;
    }
    
    if(this.connection === true) {
      if(ele_last.classList[0] === 'input' || (this.forceFirstInput && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node'))) {

        let input_class: any = "input_1";

        if(this.forceFirstInput && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node')) {
       ...
       var output_id = this.elementSelected.parentElement.parentElement.id;
       var output_class = this.elementSelected.classList[1];
        ...
        if(output_id !== input_id && input_class !== false) {
          ...
          this.connectionElement.classList.add(output_class);
          this.connectionElement.classList.add(input_class);
          var id_input = input_id.slice(5);
          var id_output = output_id.slice(5);

          this.drawFlow[this.module].data[id_output].outputs[output_class].connections.push( {"node": id_input, "output": input_class});
          this.drawFlow[this.module].data[id_input].inputs[input_class].connections.push( {"node": id_output, "input": output_class});
  ...
  contextmenu(e: any): any {
    ...
    if(this.nodeSelected || this.connectionSelected) {
      ...
      if(this.connectionSelected && (this.connectionSelected.parentElement.classList.length > 1)) {
  ...
  key(e: any): any {
    ...
    if (e.key === 'Delete' || (e.key === 'Backspace' && e.metaKey)) {
      if(this.nodeSelected != null) {
        if(this.firstClick.tagName !== 'INPUT' && this.firstClick.tagName !== 'TEXTAREA' && this.firstClick.hasAttribute('contenteditable') !== true) {
          this.removeNodeId(this.nodeSelected.id);
        }
      }
      if(this.connectionSelected != null) {
  ...
  updateConnection(eX: number, eY: number) {
    ...
    var path = this.connectionElement.children[0];

    var line_x = this.elementSelected.offsetWidth/2 + (this.elementSelected.getBoundingClientRect().x - precanvas.getBoundingClientRect().x ) * precanvasWitdhZoom;
    var line_y = this.elementSelected.offsetHeight/2 + (this.elementSelected.getBoundingClientRect().y - precanvas.getBoundingClientRect().y ) * precanvasHeightZoom;
  ...
  addConnection(id_output: any, id_input: any, output_class: any, input_class: any) {
    ...
    if(nodeOneModule === nodeTwoModule) {
      ...
      if(exist === false) {
        //Create Connection
        this.drawFlow[nodeOneModule].data[id_output].outputs[output_class].connections.push( {"node": id_input.toString(), "output": input_class});
        this.drawFlow[nodeOneModule].data[id_input].inputs[input_class].connections.push( {"node": id_output.toString(), "input": output_class});
  ...
  updateConnectionNodes(id: any) {
    ...
    const reroute_curvature = this.rerouteCurvature;
    const reroute_curvature_start_end = this.rerouteCurvatureStartEnd;
    const reroute_fix_curvature = this.rerouteFixCurvature;
    ...
    Object.keys(elemsOut).map(function(item, index) {
      if(elemsOut[item].querySelector('.point') === null) {
        ...
        points.forEach((item: any, i: number) => {
  ...
  createReroutePoint(ele: any) {
      ...
      const output_class = this.connectionSelected.parentElement.classList[3];
      const input_class = this.connectionSelected.parentElement.classList[4];
      this.connectionSelected = null;
      const point = document.createElementNS('http://www.w3.org/2000/svg',"circle");
      point.classList.add("point");
      let pos_x = this.posX * ( this.precanvas.clientWidth / (this.precanvas.clientWidth * this.zoom)) - (this.precanvas.getBoundingClientRect().x * ( this.precanvas.clientWidth / (this.precanvas.clientWidth * this.zoom)));
      let pos_y = this.posY * ( this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom)) - (this.precanvas.getBoundingClientRect().y * ( this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom)));
      ...
      if (this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points === undefined)  {
        this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points = [];
      }

      if (this.rerouteFixCurvature) {

        if (position_add_array_point > 0 || this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.length > 0) {
          this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.splice(position_add_array_point, 0, { pos_x: pos_x, pos_y: pos_y });
        } else {
          this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.push({ pos_x: pos_x, pos_y: pos_y });
        }

        ele.parentElement.querySelectorAll(".main-path").forEach((item: any, i: number) => {
          item.classList.remove("selected");
        });
  ...
  removeReroutePoint(ele: any) {
    ...
    this.drawFlow[this.module].data[nodeId].outputs[output_class].connections[searchConnection].points.splice(numberPointPosition,1);
  ...
  getNodeFromId(id: any) {
    ...
    return JSON.parse(JSON.stringify(this.drawFlow[moduleName].data[id]));
  }
  ...
  addNode (name: string, num_in: number, num_out: number, ele_pos_x: number, ele_pos_y: number, classoverride: string, data: any, html: any, typenode = false) {
    var newNodeId: any;
    if (this.useuuid) {
    ...
    if(typenode === false) {
      ...
      if(parseInt(this.render.version) === 3 ) {
        ...
        let wrapper = new this.render({
          parent: this.parent,
          render: (h: any) => h(this.nodeRegister[html].html, { props: this.nodeRegister[html].props }),
    ...
    function insertObjectkeys(object: any, name: string, completname: string) {
      if(object === null) {
    ...
    this.drawFlow[this.module].data[newNodeId] = json;
  ...
  addNodeImport (dataNode: any, precanvas: any) {
    ...
    if(dataNode.typenode === false) {
      ...
      if(parseInt(this.render.version) === 3 ) {
        ...
        let wrapper = new this.render({
          parent: this.parent,
          render: (h: any) => h(this.nodeRegister[dataNode.html].html, { props: this.nodeRegister[dataNode.html].props }),
  ...
  addRerouteImport(dataNode: any) {
    const reroute_width = this.rerouteWidth
    const reroute_fix_curvature = this.rerouteFixCurvature
    const container = this.container;
  ...
  updateNodeValue(event: any) {
    ...
    for (var i = 0; i < attr.length; i++) {
            if (attr[i].nodeName.startsWith('df-')) {
                ...
                var target = this.drawFlow[this.module].data[event.target.closest(".drawflow_content_node").parentElement.id.slice(5)].data;
  ...
  updateNodeDataFromId(id: string, data: any) {
    var moduleName = this.getModuleFromNodeId(id) ?? ''
    this.drawFlow[moduleName].data[id].data = data;
  ...
  addNodeInput(id: string) {
    ...
    this.drawFlow[moduleName].data[id].inputs["input_"+(numInputs+1)] = { "connections": []};
  }
  ...
  addNodeOutput(id: string) {
    ...
    this.drawFlow[moduleName].data[id].outputs["output_"+(numOutputs+1)] = { "connections": []};
  }
  ...
  removeNodeInput(id: string, input_class: string) {
    ...
    // Remove connections
    removeInputs.forEach((item: any, i: number) => {
      this.removeSingleConnection(item.id_output, item.id, item.output_class, item.input_class);
    });

    delete this.drawFlow[moduleName].data[id].inputs[input_class];

    // Update connection
    const connections: any[] = [];
    const connectionsInputs = this.drawFlow[moduleName].data[id].inputs
    Object.keys(connectionsInputs).map(function(key, index) {
      connections.push(connectionsInputs[key]);
    });
    this.drawFlow[moduleName].data[id].inputs = {};
    const input_class_id = input_class.slice(6);
    let nodeUpdates: any = [];
    connections.forEach((item, i) => {
      item.connections.forEach((itemx: any, f: number) => {
        nodeUpdates.push(itemx);
      });
      this.drawFlow[moduleName].data[id].inputs['input_'+ (i+1)] = item;
    });
    ...
    if(this.module === moduleName) {
      ...
      eles.forEach((item: any, i: number) => {
    ...
    nodeUpdates.forEach((itemx: any, i: number) => {
      this.drawFlow[moduleName].data[itemx.node].outputs[itemx.input].connections.forEach((itemz: any, g: number) => {
          if(itemz.node == id) {
            ...
            if(parseInt(input_class_id) < parseInt(output_id)) {
              ...
              if(itemz.points) {
                  this.drawFlow[moduleName].data[itemx.node].outputs[itemx.input].connections[g] = { node: itemz.node, output: 'input_'+(output_id-1), points: itemz.points }
              } else {
                  this.drawFlow[moduleName].data[itemx.node].outputs[itemx.input].connections[g] = { node: itemz.node, output: 'input_'+(output_id-1)}
              }
            }
          }
      });
  ...
  removeNodeOutput(id: string, output_class: string) {
    ...
    // Remove connections
    removeOutputs.forEach((item: any, i: number) => {
      this.removeSingleConnection(item.id, item.id_input, item.output_class, item.input_class);
    });

    delete this.drawFlow[moduleName].data[id].outputs[output_class];

    // Update connection
    const connections: any = [];
    const connectionsOuputs = this.drawFlow[moduleName].data[id].outputs
    Object.keys(connectionsOuputs).map(function(key, index) {
      connections.push(connectionsOuputs[key]);
    });
    this.drawFlow[moduleName].data[id].outputs = {};
    ...
    connections.forEach((item: any, i: number) => {
      ...
      this.drawFlow[moduleName].data[id].outputs['output_'+ (i+1)] = item;
    });
    nodeUpdates =  new Set(nodeUpdates.map((e: any) => JSON.stringify(e)));
    nodeUpdates = Array.from(nodeUpdates).map((e: any) => JSON.parse(e));
    ...
    nodeUpdates.forEach((itemx: any, i: number) => {
      this.drawFlow[moduleName].data[itemx.node].inputs[itemx.output].connections.forEach((itemz: any, g: number) => {
          if(itemz.node == id) {
            ...
            if(parseInt(output_class_id) < parseInt(input_id)) {
              ...
              if(itemz.points) {
                  this.drawFlow[moduleName].data[itemx.node].inputs[itemx.output].connections[g] = { node: itemz.node, input: 'output_'+(input_id-1), points: itemz.points }
              } else {
                  this.drawFlow[moduleName].data[itemx.node].inputs[itemx.output].connections[g] = { node: itemz.node, input: 'output_'+(input_id-1)}
              }
            }
          }
      });
  ...
  removeNodeId(id: string) {
    ...
    delete this.drawFlow[moduleName].data[id.slice(5)];
    this.dispatch('nodeRemoved', id.slice(5));
  }

  removeConnection() {
    if(this.connectionSelected != null) {
      var listclass = this.connectionSelected.parentElement.classList;
      this.connectionSelected.parentElement.remove();
      //console.log(listclass);
      var index_out = this.drawFlow[this.module].data[listclass[2].slice(14)].outputs[listclass[3]].connections.findIndex(function(item: any, i: number) {
        return item.node === listclass[1].slice(13) && item.output === listclass[4]
      });
      this.drawFlow[this.module].data[listclass[2].slice(14)].outputs[listclass[3]].connections.splice(index_out,1);

      var index_in = this.drawFlow[this.module].data[listclass[1].slice(13)].inputs[listclass[4]].connections.findIndex(function(item: any, i: number) {
        return item.node === listclass[2].slice(14) && item.input === listclass[3]
      });
      this.drawFlow[this.module].data[listclass[1].slice(13)].inputs[listclass[4]].connections.splice(index_in,1);
  ...
  removeSingleConnection(id_output: string, id_input: string, output_class: string, input_class: string) {
    ...
    if(nodeOneModule === nodeTwoModule) {
      ...
      // Check connection exist
      var exists = this.drawFlow[nodeOneModule].data[id_output].outputs[output_class].connections.findIndex(function(item: any, i: number) {
      ...
      if(exists > -1) {
        ...
        var index_out = this.drawFlow[nodeOneModule].data[id_output].outputs[output_class].connections.findIndex(function(item: any, i: number) {
          return item.node == id_input && item.output === input_class
        });
        this.drawFlow[nodeOneModule].data[id_output].outputs[output_class].connections.splice(index_out,1);

        var index_in = this.drawFlow[nodeOneModule].data[id_input].inputs[input_class].connections.findIndex(function(item: any, i: number) {
          return item.node == id_output && item.input === output_class
        });
        this.drawFlow[nodeOneModule].data[id_input].inputs[input_class].connections.splice(index_in,1);
  ...
  removeModule(name: string) {
    ...
    delete this.drawFlow[name];
    this.dispatch('moduleRemoved', name);
  }

  clearModuleSelected() {
    this.precanvas.innerHTML = "";
    this.drawFlow[this.module] =  { "data": {} };
  }
  ...
  on (event: any, callback: any): any {
   ...
   removeListener (event: any, callback: any): any {
      // Check if this event not exists
